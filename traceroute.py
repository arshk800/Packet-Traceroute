...
class IcmpHelperLibrary:
    ...
    class IcmpPacket:
        ...
        #                                                                                                              #
        # ############################################################################################################ #
        def getrecPacket(self):
            return self.__recvPacket

        def getserviceType(self):
            return self.__servicetype

        def gettimeReceive(self):
            return self.__timeReceive

        def getpacketReceive(self):
            return self.__packetReceive
        ...
        #                                                                                                              #
        # ############################################################################################################ #
        def setserviceType(self, booleanvalue):
            self.__servicetype = booleanvalue

        def settimeReceive(self, settime):
            self.__timeReceive = settime

        def setpacketReceive(self, booleanValue):
            self.__packetReceive = booleanValue

        def setisValid(self, booleanValue):
            self.__isValid = booleanValue
        ...
        def __validateIcmpReplyPacketWithOriginalPingData(self, icmpReplyPacket):
            ...
            errMessage = ''
            if icmpReplyPacket is not None:
                self.setpacketReceive(True)

            if icmpReplyPacket.getIcmpSequenceNumber() == self.getPacketSequenceNumber():
                icmpReplyPacket.setisValidSequenceNumber(True)
            else:
                errMessage += "Error: Invalid Sequence Number\n"
                errMessage += f"Expected Sequence Number: {self.getPacketSequenceNumber()}\n"
                errMessage += f"Received: {icmpReplyPacket.getIcmpSequenceNumber()}"

            if icmpReplyPacket.getIcmpIdentifier() == self.getPacketIdentifier():
                icmpReplyPacket.setisValidIdentifier(True)
            else:
                errMessage += "Error: Invalid Identifier Number\n"
                errMessage += f"Expected Identifier Number: {self.getPacketIdentifier()}\n"
                errMessage += f"Received: {icmpReplyPacket.getIcmpIdentifier()}"

            if icmpReplyPacket.getIcmpData() == self.__dataRaw:
                icmpReplyPacket.setisValidData(True)
            else:
                errMessage += "Error: Invalid Data\n"
                errMessage += f"Expected Data: {self.__dataRaw}\n"
                errMessage += f"Received: {icmpReplyPacket.getIcmpData()}"

            # if everything as expected, set the Response as True
            if icmpReplyPacket.getisValidSequenceNumber() and icmpReplyPacket.getisValidIdentifier() \
                    and icmpReplyPacket.getisValidData():
                icmpReplyPacket.setIsValidResponse(True)
                self.setisValid(True)

            # store any error messages recorded
            icmpReplyPacket.seterrMessage(errMessage)
        ...
        #                                                                                                              #
        # ############################################################################################################ #
        def helper_icmp_codes(self, icmptype, icmpCode):
            # Source: https://www.iana.org/assignments/icmp-parameters/icmp-parameters.xhtml#icmp-parameters-codes-0
            """ :parameter icmptype and icmpcode for parsing the relevant code message
                    return the specific code message for the ICMP type """
            if icmptype == 3:
                codes = {
                    0: "Net Unreachable",
                    1: "Host Unreachable",
        ...
        def sendEchoRequest(self):
            ...
            try:
                ...
                self.__timeReceive = timeReceived
                ...
                if timeLeft <= 0:
                    ...
                    codeMessage = self.helper_icmp_codes(icmpType, icmpCode)
    ...
    class IcmpPacket_EchoReply:
        ...
        def seterrMessage(self, message):
            self.__errMessage = message

        def setisValidData(self, booleanValue):
            self.__isValidData = booleanValue
        ...
        def setisValidSequenceNumber(self, booleanValue):
            self.isValidSequenceNumber = booleanValue
        ...
        def printResultToConsole(self, ttl, timeReceived, addr):
            ...
            # if any error messages are recorded
            if not self.isValidResponse():
                print(self.geterrMessage())
                self.seterrMessage('')
            ...
            self.setrtt(round((timeReceived - timeSent) * 1000, 3))
    ...
    def __sendIcmpEchoRequest(self, host):
        ...
        for i in range(4):
            ...
            icmpPacket.setserviceType(True)  # prints the relevant Pinging information line
            ...
            sent += 1
            bytes = struct.calcsize("d")
            timeSent = struct.unpack("d", icmpPacket.getrecPacket()[28:28 + bytes])[0]

            # timer and packet received information getters in IcmpPacket
            endTimer = icmpPacket.gettimeReceive()
            rtt = round((endTimer - timeSent) * 1000, 3)

            if icmpPacket.getpacketReceive():
                receive += 1
                rttList.append(rtt)

        print(f"----------------------{host} Ping Statistics----------------------")
        print(f"Packet(s) Sent:{sent}, Packet(s) Received:{receive}")
        print(f"Packet Loss: {(((sent - receive) / sent) * 100):.3f}%")
        if rttList:
            print(f"Min RTT : {min(rttList):.3f} ms, Max RTT: {max(rttList):.3f} ms, Average: {(sum(rttList) / len(rttList)):.3f} ms")
        else:
            print("No RTTList")
        rttList.clear()
    ...
    def __sendIcmpTraceRoute(self, host):
        ...
        for ttl in range(1, maxHops + 1):
            ...
            # stop traceroute if we have reached destination or maxHops
            if icmpPacket.getaddr() == host and icmpPacket.getIcmpType() == 0:
                return
            if ttl > maxHops:
                return
...
def main():
    ...
    # icmpHelperPing.sendPing("www.uni-qiessen.de")
    #icmpHelperPing.sendPing("gaia.cs.umass.edu")
    #icmpHelperPing.traceRoute("164.151.129.20")
    #icmpHelperPing.traceRoute("122.56.99.243")
    #icmpHelperPing.traceRoute("1.159.255.255")         # Australia
    #icmpHelperPing.traceRoute("192.0.2.1")
    #icmpHelperPing.traceRoute("200.10.227.250")        # Brazil
    #icmpHelperPing.traceRoute("134.176.3.22")           # Europe
